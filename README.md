...
On GitHub, the [Docker workflow](.github/workflows/docker-ghcr.yml) builds and pushes to **GHCR** on pushes to the default branch and version tags (`v*`). The image name is `ghcr.io/<github-owner>/immichmcp` (owner and name are lowercased), with a `latest` tag on the default branch.

```bash
docker run -e IMMICH_BASE_URL="https://photos.example.com" \
           -e IMMICH_API_KEY="your-api-key" \
           -p 5000:5000 \
           ghcr.io/<your-github-username>/immichmcp:latest
```
  ...
  "mcpServers": {
    "immich": {
      ...
      "args": ["run", "-i", "--rm",
               ...
               "-e", "IMMICH_API_KEY=your-api-key",
               "ghcr.io/<your-github-username>/immichmcp:latest", "--stdio"]
    }
  }
}
```
